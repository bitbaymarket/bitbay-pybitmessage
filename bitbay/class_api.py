...
def getAPI(workingdir=None,silent=False):
    ...
    class MainAPI(bitmessagemain.Main):
        ...
        def getAllInboxUnreadMessages(self):

            '''Return a List of all Inbox Unread Messages
            Usage: api.getAllInboxMessages()'''

            queryreturn = bitmessagemain.shared.sqlQuery('''SELECT msgid, toaddress, fromaddress, subject, received, message, encodingtype, read FROM inbox WHERE folder='inbox' AND read='0' ORDER BY received''')
            messages = []
            for row in queryreturn:
                msgid, toAddress, fromAddress, subject, received, message, encodingtype, read = row
                subject = bitmessagemain.shared.fixPotentiallyInvalidUTF8Data(subject)
